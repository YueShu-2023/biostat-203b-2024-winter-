...
patient_title <- paste("Patient ", patients$subject_id[1], 
                     patients$Gender, 
                       patients$Age, 
                       " years old, ", patients$Race, sep = ",")
...
ggplot() +
  ...
  geom_point(data = procedures_icd, aes(x = as.POSIXct(chartdate), y = "Procedure", shape=long_title)) +
  ...
  labs(title = paste("Patient",10013310,",",patients$gender,",",patients$anchor_age,"years old",tolower(admissions$race)), subtitle = diagnoses_icd,
       x = "Time", y = "Event Type", color = "Care Unit") 
...
#| eval=False
chartevents <- read_parquet("~/chartevents_pq/part-0.parquet")%>%
  filter(subject_id == 10013310)%>%
  group_by(stay_id,itemid)%>%
  mutate(charttime = as.POSIXct(charttime))

patient_title2 <- paste("Patient ",10013310, 
                       " ICU stays - Vitals ", sep = "")

ggplot() +
  geom_line(data = chartevents, aes(x=charttime), y = "itenid",
             shape = icd_code)) 
...
# Read chartevents.pq and filter relevant vitals
...
# Filter lab events for patients in icustays_tble
joint_tble <- chartevents_tble %>%
  inner_join(icustays_tble, by =c( "subject_id","hadm_id", "stay_id"))%>%
  filter(charttime >= intime & charttime <= outtime)

# Convert storetime to POSIXct
joint_tble <- joint_tble %>%
  mutate(storetime = as.POSIXct(storetime))

# Group by subject_id and itemid, and filter the last available measurement before the ICU stay
joint_tble2 <- joint_tble %>%
  group_by(subject_id, stay_id,itemid) %>%
  summarise(value = first(value, order_by = charttime), .groups = "drop")%>%
  ungroup() %>%
  pivot_wider(names_from = itemid, values_from = value,names_prefix = "vital_")
...
all_join <- inner_join(icustays_tble, admissions_tble, by = "subject_id") %>%
                 inner_join(patients_tble, by = "subject_id") %>%
                 mutate(subject_id = as.character(subject_id)) 

mimic_icu_cohort <- all_join %>%
